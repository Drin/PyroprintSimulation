...
DEV_HELP      = 'The maximum number of GPU cards to be used for computation'
PYRO_LEN_HELP = 'The cut-off length of the pyrosequence'
 ...
 * num_loci  - The number of loci (replicate ITS regions) to simulate for each
               isolate
 * device    - The number of GPU devices to use
...
def parse_args(programDesc):
   ...
   parser.add_argument("--pyro_len", dest="pyro_len",  default=0,type=int, help=PYRO_LEN_HELP)
   ...
   if (args.get('file') is not None):
      ...
      if config.has_option("params", "pyro_len"):
         args['pyro_len'] = config.getint("params", "pyro_len")

      if config.has_option("params", "num_gpus"):
         args['num_gpus'] = config.getint("params", "num_gpus")
